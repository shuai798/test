...
@connect(({ dataCockpitSpace, loading }) => {
    return {
        dataCockpitSpace,
        loading,
    };
...
class MapChart extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.deviceRestData !== prevState.deviceRestData) {
            return { deviceRestData: nextProps.deviceRestData };
        }
        return null;
    }

    getOption = (provinceValue) => {
        const { provinceData, deviceRestData } = this.state;
        let value = china;
        provinceData.map((item) => {
            if (item.id === provinceValue) {
                value = item.name;
            }
        });
        echarts.registerMap(provinceValue, value);
        const geoCoordMap = {
        ...
        const mapName = provinceValue;
        const res = [];
        const endsValue = ['省', '维吾尔自治区', '回族自治区', '自治区', '壮族自治区'];
        for (let i = 0; i < deviceRestData.length; i += 1) {
            for (let j = 0; j < endsValue.length; j += 1) {
                if (deviceRestData[i].name.endsWith(endsValue[j])) {
                    const k = deviceRestData[i].name.indexOf(endsValue[j]);
                    deviceRestData[i].name = deviceRestData[i].name.substring(0, k);
                }
                if (deviceRestData[i].code.indexOf('P110000') !== -1) {
                    deviceRestData[i].name = '北京';
                }
                if (deviceRestData[i].code.indexOf('P310000') !== -1) {
                    deviceRestData[i].name = '上海';
                }
                if (deviceRestData[i].code.indexOf('P120000') !== -1) {
                    deviceRestData[i].name = '天津';
                }
                if (deviceRestData[i].code.indexOf('P500000') !== -1) {
                    deviceRestData[i].name = '重庆';
                }
            }
            deviceRestData[i].value = deviceRestData[i].remainingRunningTimes;
            const geoCoord = geoCoordMap[deviceRestData[i].name];
            if (geoCoord) {
                res.push({
                    name: deviceRestData[i].name,
                    value: geoCoord.concat(deviceRestData[i].remainingRunningTimes),
                });
            }
        }
        const option = {
    ...
    render() {
        ...
        return (
            <div className={styles.flowTrendBox}>
                <ReactEcharts
                    option={this.getOption(provinceValue)}
                    style={{
                        width: '100%',
                        height: '100%',
                    }}></ReactEcharts>
