...
class CustomerBarChart extends Component {
    ...
    shouldComponentUpdate(nextProps) {
        if (nextProps.top30Data !== this.state.top30Data) {
            return true;
        }
        return false;
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.top30Data !== prevState.top30Data) {
            return { top30Data: nextProps.top30Data };
        }
        return null;
    }
    ...
    initOption = () => {
        ...
        (top30Data || []).map((item) => {
            areaName.push(item.industryName);
            accessDeviceNum.push(item.customerNum);
        });
        const option = {
            ...
            series: [
                ...
                {
                    name: '客户数量', // 系列名称，用于tooltip的显示，legend 的图例筛选，在 setOption 更新数据和配置项时用于指定对应的系列。
                    barWidth: 16,
                    barGap: '30%',
