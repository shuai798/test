...
class BarChart extends Component {
    ...
    shouldComponentUpdate(nextProps) {
        if (nextProps.troubleType !== this.state.troubleType) {
            return true;
        }
        return false;
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.troubleType !== prevState.troubleType) {
            return { troubleType: nextProps.troubleType };
        }
        return null;
    }
    ...
    initOption = () => {
        ...
        const xData = []; // 故障类型
        const num = []; // 故障数量
        if (troubleType && troubleType.length > 0) {
        ...
        const option = {
            ...
            series: [
                // 系列列表。每个系列通过 type 决定自己的图表类型
                {
                    ...
                    barWidth: 16,
                    barGap: '30%',
                    data: num, // 系列中的数据内容数组。数组项通常为具体的数据项。
                    type: 'bar', // 类型。
                    color: '#5b8ff9',
                    /**
