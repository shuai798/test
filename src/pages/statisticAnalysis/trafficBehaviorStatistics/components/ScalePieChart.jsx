...
class PieChart extends Component {
    ...
    shouldComponentUpdate(nextProps) {
        if (nextProps.troubleModel !== this.state.troubleModel) {
            return true;
        }
        return false;
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.troubleModel !== prevState.troubleModel) {
            return { troubleModel: nextProps.troubleModel };
        }
        return null;
    }
    ...
    initOption = () => {
        ...
        (troubleModel || []).map((item) => {
            name.push(item.seriesName);
            item.name = item.seriesName;
            item.value = item.num;
        });
        let itemStyle = {};
        let data = [];
        if (troubleModel && troubleModel.length > 0) {
            troubleModel.map((item) => {
                name.push(item.seriesName);
                item.name = item.seriesName;
                item.value = item.num;
        ...
        const option = {
            ...
            color: ['#6395f9', '#62daab', '#657798', '#f6c022', '#e96c5b', '#74cbed'],
            textStyle: {
                color: '#999999',
            },
            tooltip: {
