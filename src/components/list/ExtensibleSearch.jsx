...
import { Row, Col, Form, Input, Button, Select, Icon, DatePicker } from 'antd';
...
class ExtensibleSearch extends Component {
    ...
    handleFormSearch = e => {
        ...
        form.validateFields((err, value) => {
            if (err) { return; }
            // 转换数据格式，方便后端接收
            if (value.time && value.time.length === 2) {
                value.startTime = `${value.time[0].format('YYYY-MM-DD')} 00:00:00`;
                value.endTime = `${value.time[1].format('YYYY-MM-DD')} 00:00:00`;
            }
            delete value.time;
